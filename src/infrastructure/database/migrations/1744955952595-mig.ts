...
export class Mig1744955952595 implements MigrationInterface {
    name = 'Mig1744955952595'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`transaction\` (\`transactionId\` int NOT NULL AUTO_INCREMENT, \`status\` varchar(255) NOT NULL DEFAULT 'INITIATED', \`paymentProvider\` varchar(255) NOT NULL, \`remarks\` varchar(255) NULL, \`paymentProviderId\` varchar(255) NULL, \`amount\` int NULL, \`date\` datetime NOT NULL, \`createdAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0), \`updatedAt\` timestamp(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(0) ON UPDATE CURRENT_TIMESTAMP(0), \`userUserId\` varchar(36) NOT NULL, PRIMARY KEY (\`transactionId\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`transaction\` ADD CONSTRAINT \`FK_ae172769e4ad93ccd65fdbe3407\` FOREIGN KEY (\`userUserId\`) REFERENCES \`user\`(\`userId\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`transaction\` DROP FOREIGN KEY \`FK_ae172769e4ad93ccd65fdbe3407\``);
